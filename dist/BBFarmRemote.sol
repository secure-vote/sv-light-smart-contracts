...
library RLPEncode {
    ...
    function encodeBytes(bytes memory self) internal view returns (bytes) {
    ...
    /// @param self The bytes[] to be encoded
    /// @return The rlp encoded bytes[]
    function encodeList(bytes[] memory self) internal view returns (bytes) {
        bytes memory list = flatten(self);
        bytes memory encoded = encode(list, LIST_SHORT_PREFIX, LIST_LONG_PREFIX);
        return encoded;
    }

    function encode(bytes memory self, uint8 prefix1, uint8 prefix2) private view returns (bytes) {
        uint selfPtr;
    ...
    function flatten(bytes[] memory self) private view returns (bytes) {
        if(self.length == 0) {
            return new bytes(0);
        }
    ...
    function memcpy(uint dest, uint src, uint len) private view {
